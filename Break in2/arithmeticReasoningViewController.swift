...
//**************************************************************************************
    ...
    override func viewDidLoad() {
        ...
        if self.showTutorial == true {
            ...
            //Tutorial Title
            let labelString:String = String("ARITHMETIC TEST")
            ...
            if self.interstitialAd.isReady && self.membershipType == "Free" {
                ...
                self.timeTimer = Timer.scheduledTimer(timeInterval: 1.0, target: self, selector: #selector(arithmeticReasoningViewController.updateTimer), userInfo: nil, repeats: true)
            }
        }
    ...
    func displayQuestion(_ indexQuestion:Int) {
      ...
      for i:Int in stride(from: 0, to: arrayAnswers.count, by: 1) {
            ...
            answerNumber.setTitle(arrayAnswers[i], for: UIControlState())
            ...
            answerNumber.titleLabel?.font = UIFont(name: "HelveticaNeue-Bold", size: self.view.getTextSize(22))
            ...
            matchingQuestionLabel.titleLabel?.font = UIFont(name: "HelveticaNeue-Bold", size: self.view.getTextSize(28))
            matchingQuestionLabel.setTitle("\(questionAsked) =", for: UIControlState())
            ...
            if self.displayedQuestionIndex + 1 > self.totalNumberOfQuestions {
                ...
                if self.resultsUploaded==false {
                    ...
                    var nbCorrectAnswers:Int = 0
                      for i:Int in stride(from: 0, to: self.quizzArray.count, by: 1) {
...
//**************************************************************************************
    ...
    func feedbackScreen() {
      ...
      for i:Int in stride(from: 0, to: self.selectedAnswers.count, by: 1) {
            ...
            let topMargin:NSLayoutConstraint = NSLayoutConstraint(item: answerUIButton, attribute: NSLayoutAttribute.top, relatedBy: NSLayoutRelation.equal, toItem: self.feebdackScreen, attribute: NSLayoutAttribute.top, multiplier: 1, constant: CGFloat(i*(buttonHeight+10) + Int(20*self.heightRatio)))
    ...
    func fillArrayWithRandomNumbers(_ operation:String) -> (String, [String],Int) {
        ...
        if operation=="+" {
          ...
          for i:Int in stride(from: 0, to: 5, by: 1) {
        ...
        for i:Int in stride(from: 0, to: 6, by: 1) {
    ...
    func createAndLoadInterstitial() -> GADInterstitial {
        let interstitial = GADInterstitial(adUnitID: AD_ID_ARITHMETIC)
